...
class DeformableTransformer(nn.Module):
    ...
    def forward(self, samples, targets, srcs, masks, refpoint_embed, pos_embeds, tgt, attn_mask=None):
        """
        Input:
        ...
        if self.two_stage_type =='standard':
            ...
            # enc_outputs_coord_unselected = modify_init_box_proposals(enc_outputs_coord_unselected, targets)
            topk = self.num_queries
            topk_proposals = torch.topk(enc_outputs_class_unselected.max(-1)[0], topk, dim=1)[1] # bs, nq
            # topk_proposals = torch.arange(topk, device=enc_outputs_class_unselected.device).unsqueeze(0).repeat(enc_outputs_class_unselected.size(0), 1)
        ...
        # refpoints = refpoint_embed[0].sigmoid()  # Sigmoid to get normalized coordinates
